...
DoubleMatrix MssmSusy::displayYukawaMatrix(yukawa k) const {
  switch(k) {
...
DoubleVector MssmSusy::display() const {
...
void setBetas(DoubleMatrix & babBeta, DoubleVector &cuBeta, DoubleVector
	      & cdBeta, DoubleVector & ceBeta, DoubleVector & bBeta , int N ) {
  // 1 loop gauge beta fns
  bBeta(1) = 33.0 / 5.0; bBeta(2) = 1.0; bBeta(3) = -3.0; 
  

  //  cout << "numofmessenger = " << N << endl;  // IWKIM
  if( N != 0 ) { 
    //   cout << "Changing beta function" << endl; 
    bBeta(1) = bBeta(1) + N ; 
    bBeta(2) = bBeta(2) + N ; 
    bBeta(3) = bBeta(3) + N ; 
...
void MssmSusy::anomalousDimension(DoubleMatrix & gEE, DoubleMatrix & gLL,
  ...
  if (bBeta(1) < 1.0e-5) // Constants not set yet
    setBetas(babBeta, cuBeta, cdBeta, ceBeta, bBeta, N );
