...
{  char * command;
   ...
   sprintf(command,
    "lPath=%s;EXPCON_PATH=$lPath/NMSSMTools_2.1.0/EXPCON;export EXPCON_PATH;$lPath/%s"
    ,LPATH,cmd); 
   system(command);
   free(command);
...
{  int err;
   int del=getdelfilesstat_();
   err=sugraLesH("inp.dat", m0,mhf,a0,tb, sgn,Lambda, aLambda,  aKappa);
   if(err) return -1; 
   err=runTools("nmspec","spectr.dat",1);
