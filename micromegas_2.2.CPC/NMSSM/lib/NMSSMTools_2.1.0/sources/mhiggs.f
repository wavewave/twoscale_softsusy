...
************************************************************************
	...
	DOUBLE PRECISION XIFSUSY,XISSUSY,MUPSUSY,MSTSUSY,M3SUSY
	DOUBLE PRECISION XIF,XIS,MUP,MST,M3H
	...
	COMMON/GMSUSYPAR/XIFSUSY,XISSUSY,MUPSUSY,MSTSUSY,M3SUSY
...
*   Approximate value for the Singlet-like CP odd Higgs mass squared:
	MP2=-3.D0*nuq*Akq
     C   -XIF*(4.D0*KQ+2.D0*LQ*MUP/MUQ)-4.D0*MST-2.D0*MUP*NUQ-LQ*XIS/MUQ
...
*   Tree level CP-even Higgs mass matrix
	...
	GMCOMB=Lq*XIF+2.D0*MUP*MUQ+M3H

	H(1,1) = gq*h1q**2 + (muq*B+GMCOMB)/tanbq
	H(2,2) = gq*h2q**2 + (muq*B+GMCOMB)*tanbq
	H(3,3) = Lq**2*(Alshift+2.D0*MUP)*h1q*h2q/muq 
     C   + nuq*(AKQ+4.D0*NUQ+6.D0*MUP)-LQ/MUQ*(XIS+2.D0*XIF*MUP)
	H(1,2) = (2.D0*Lq**2-gq)*h1q*h2q - muq*B-GMCOMB
	H(1,3) = Lq*(2.D0*muq*h1q - (B+nuq+2.D0*MUP)*h2q)
	H(2,3) = Lq*(2.D0*muq*h2q - (B+nuq+2.D0*MUP)*h1q)
...
*    b) Chargino/neutralino contributions
	...
	Lnu= DLOG(MAX(4.D0*(nuq+MUP)**2,MZ**2)/QSTSB)
	Lmunu= DLOG(MAX(muq**2,4.D0*(nuq+MUP)**2,MZ**2)/QSTSB)

	H(1,1)= H(1,1) + COEF*
     C	 ((MZ**2*SB**2*Gq*(-20.D0+32.D0*S2TW-16.D0*S2TW**2))*LM2MU
     C	  -4.D0*(MUq*(NUq+MUP)*Lq**2/TANBQ+MZ**2*SB**2*Lq**4/Gq)*LMUNU)

	H(2,2)= H(2,2) + COEF*
     C	 ((MZ**2*CB**2*Gq*(-20.D0+32.D0*S2TW-16.D0*S2TW**2))*LM2MU
     C	  -4.D0*(MUq*(NUq+MUP)*Lq**2*TANBQ+MZ**2*CB**2*Lq**4/Gq)*LMUNU)

	H(3,3)= H(3,3) + COEF*
     C	 (16.D0*KQ**2*(-2.D0*NUq**2-3.D0*NUq*MUP+MUP**3/NUq)*LNU
     C   -8.*Lq**2*MUq**2*LMU
     C   +Lq**3*MZ**2*MUP/(MUQ*GQ)*(8.D0*KQ-2.D0*LQ*S2)*Lmunu)

	H(1,2)= H(1,2) + COEF*
     C	 (4.D0*(MUq*(NUq+MUP)*Lq**2-MZ**2*SB*CB*Lq**4/Gq)*LMUNU
     C	 -(4.D0*Gq*MZ**2*SB*CB)*LM2MU)

	H(1,3)= H(1,3) + COEF*(MZ/DSQRT(Gq))*
     C	 (Lq*Gq*MUq*SB*(-12.D0+8.D0*S2TW)*LM2MU
     C	 +4.D0*Lq**2*(CB*LQ*(2.D0*NUQ+MUP)-SB*(LQ*MUQ
     C   +4.D0*KQ*(NUQ+MUP)))*LMUNU)
 
	H(2,3)= H(2,3) + COEF*(MZ/DSQRT(Gq))*
     C	 (Lq*Gq*MUq*CB*(-12.D0+8.D0*S2TW)*LM2MU
     C	 +4.D0*Lq**2*(SB*LQ*(2.D0*NUQ+MUP)-CB*(LQ*MUQ
     C   +4.D0*KQ*(NUQ+MUP)))*LMUNU)
...
*   1-loop top/bottom radiative corrections
	...
	A(2,2)= LQ**2*h1q*h2q/MUQ*(3.D0*NUQ+B)-3.D0*AKQ*NUQ
     C	 -XIF*(4.D0*KQ+2.D0*LQ*MUP/MUQ)-4.D0*MST-2.D0*MUP*NUQ-LQ*XIS/MUQ
	A(1,2)= LQ*(Alshift-2.D0*NUQ-2.D0*MUP)
