...
**********************************************************************
	...
	DOUBLE PRECISION B,REW,MUOLD,KOLD,MXSOLD
	...
	DOUBLE PRECISION Q2,MH1X,MH2X,MSSX,MH1P,MH2P
...
* equations, included as a shift in MH1/MH2 from MH1X/MH2X to 
* MH1P/MH2P:
 
	 REW=COEF*GQ*KQ/LQ*(2.D0*S2TW-3.D0)
     C      *DLOG(MAX(M2**2,MUOLD**2,MZ**2)/QSTSB)

	 MH1P=MH1X+REW*MUOLD**2/TANBQ    
  	 MH2P=MH2X+REW*MUOLD**2*TANBQ
...
* Prepare the computation of MU:
	...
	cm=gq/2.D0*(H1Q**2+H2Q**2)*(tanbq-1.D0/tanbq)
     c     +(rt1+mh1P)*tanbq-(rb1+mh2P)/tanbq
...
*  K at QSTSB
	...
	B=TANBQ/((1.D0+TANBQ**2)*MUQ)*(MH1P+MH2P+2.D0*MUQ**2
     C	  +LQ**2*(H1Q**2+H2Q**2)+RT1+RB1-MUQ*(RT2*TANBQ+RB2/TANBQ))
	KQ=LQ*(B-ALQ-2.D0*MUP-(LQ*XIF+M3H)/MUQ)/MUQ	
...
*  MSSX = MS at QSTSB, stored in COMMON/QMHIGGS:
	...
	MSSX=-LQ**2*(H1Q**2+H2Q**2) - 2.D0*NUQ**2
     C	     +LQ**2*H1Q*H2Q/MUQ*(ALQ+2.D0*NUQ+2.D0*MUP) - NUQ*AKQ
     C	     -2.D0*KQ*XIF*(1.D0+MUP/NUQ)-4.D0*MUP**2-6.D0*MUP*NUQ
     C	     -2.D0*MST-LQ*XIS/MUQ
...
*  NEW XIS:
	...
	XIS=LQ*(H1Q*H2Q*(ALQ+2.D0*NUQ+2.D0*MUP)-MUQ*(H1Q**2+H2Q**2))
     C	    -MUQ/LQ*(MSSX+2.D0*NUQ**2+NUQ*AKQ+2.D0*KQ*XIF*(1.D0+MUP/NUQ)
     C	    +4.D0*MUP**2+6.D0*MUP*NUQ+2.D0*MST
     C	    -ct*LQ**2*H1Q*H2Q/MUQ*Xt*gmt
     C	    -cb*LQ**2*H1Q*H2Q/MUQ*Xb*gmb)
