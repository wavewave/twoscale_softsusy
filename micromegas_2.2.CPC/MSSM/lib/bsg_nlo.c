...
static	double  alphem = /*0.0077579519*/ 1.0/137.036;
static	double alphew = 0.007819/*1.0/127.9*/;
...
/*-----------------------------------------------------------*/
	...
	double alpsmt,alpsmw,delmt,mtmw2,eta_wmu,eta_wmb;
	double c70smw,c70Hw,c80smw,c80Hw;
	double mbs,lam2,mub,mubar,del,ckmf;
...
/* Set the renormalisation scale mubar, this value is the one relevant for semileptonic B decays, can be changed between Mbpole/2 to 2 MbPole*/
	mubar=MbPole;
	ckmf=param.Vts*param.Vtb/param.Vcb;
...
*/	
	...
	eta_wmb=alpsmw/alps(mb,param.MZ,alpsmz);
...
/*   printf("calc_eps inside of deltaMb =%E\n",eps_b); */
...
double bsgnlo_(void)
{
	...
	double c7_epsb,c8_epsb,c7_epsbh,c8_epsbh;
	double c7_chi_mw, c7_su, c8_chi_mw, c8_su;
	double c7_hp_mw, c8_hp_mw, eta_h,alpshp;
	double c2_sum;
	...
	double c80_mu,c70_mu,c71_w,c81_w,c71_sum,c81_tum,c71_wmu;
	double c7em_mu,cem8_mu,cem2_mu;
	double mbs,lam2,kap,rer2,rer8,r7,mub,mubar,del,ckmf;
	double K77,K27,K78,K22,K88,K28,kslem_mub;
	double kap1,ga77,g27,g87,S_mub,facbsg,bsg_lo,Knlo,bsg_nlo;
#ifdef DEBUG
        double c70suw,c80suw,c78_sm_tbcorr,c78_hp_tbcorr;
...
/* Runnning from mw to mub  : SM+HIGGS+SUSY*/
	...
	c70_mu=c70_w*pow(eta_wmu,(16./23.))+8.*(pow(eta_wmu,(14./23.))								-pow(eta_wmu,(16./23.)))*c80_w/3.+c2_sum;
...
/**  The  K_ij functions */
	...
	K77=S_mub*(1+alps(mub,param.MZ,alpsmz)*(r7+ga77*log(mb/mub)-16/3.)/2./M_PI
	 +6.*(pow((1-z0),4)/f(z0)-1)*lam2/mb/mb)+alps(mub,param.MZ,alpsmz)*
...
/* In K27,K28,K88, the numerical values correspond to delta=.9 and z0=.29^2*/
	...
	K27=S_mub*(alps(mub,param.MZ,alpsmz)*(rer2+g27*log(mb/mub))/2./M_PI
	 -lam2/9./mc/mc)+  alps(mub,param.MZ,alpsmz)*(f27(z1))/M_PI;

	K78=S_mub*alps(mub,param.MZ,alpsmz)*(rer8+g87*log(mb/mub))/2./M_PI
...
/*	K22=alps(mub,MZ,alpsmz)*f22(z0,del)/M_PI;*/
	...
	facbsg=6*.1045*alphem*ckmf*ckmf/M_PI/f(z0);
