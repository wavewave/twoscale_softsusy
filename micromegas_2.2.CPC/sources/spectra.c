...
{ static int rdOk=0;
  ...
  FILE *f;
  char * buff;
  char *fnames[Nin]={"gg.dat","dd.dat","uu.dat","ss.dat","cc.dat","bb.dat","tt.dat",
                      "ee.dat","mm.dat","ll.dat","ww.dat","zz.dat"};
...
static void getSpectrum(double M, double m1,double m2,char*n1,char*n2, long N1, long N2,int outP, double *tab)
{
  ...
  { N=labs(N1);
    ...
    { case 21:inP=0; break;  /*glu*/ 
      ...
      case 24:inP=10; break;  /*w*/
      case 23:inP=11; break;  /*z*/
    }
  }
    ...
    { double tabAux[NZ];
      ...
      {  
         ...
         { int outPpdg;
           ...
           { i=NZ*log(2*p2/M)/LnXmin -0.5;    if(i<0) i=0; tab[i]-=NZ/LnXmin; } 
           continue;
         } 
      }
      if(inP>=0)
      ...
      { double w=0;
        ...
        d2Proc=newProcess_(0,1,ForceUG,process,NULL,NULL,plib,0);
        if(!d2Proc) continue;
...
{ int n,i,err;
  ...
  {  sprintf(lib,"omg_%s%s",nameL,anameL);
     ...
     {  vcs_=calcSpectrum0(v,cc, outP,tab_);
        if(vcs_) for(i=0;i<NZ;i++)tab[i]=(tab[i]*vcs+tab_[i]*vcs_)/(vcs+vcs_);
        vcs=(vcs+vcs_)/2;         
     } 
  }
  return vcs; 
...
static double Halo_integrand_F(double x)
{
  ...
  if(R<rMin) return 0;
  rho=rhoQ_stat_F(&R)/x/x; 
  if(R<1.5*rMin) return  2*(R/rMin-1)*rho; else return rho;
}
