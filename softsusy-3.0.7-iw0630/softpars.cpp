...
/* This is the OLD VERSION of AMSB, corrected - was used as a check on the new
   ones. 
void SoftParsMssm::addAmsb(double m32) {
  MssmSusy run(displaySusy());
  const double ONEO16pisq = 1.0 / (16. * sqr(PI));
  ...
  double 
    ...
    yc   = run.displayYukawaElement(YU, 2, 2), 
    ys   = run.displayYukawaElement(YD, 2, 2), 
    ymu  = run.displayYukawaElement(YE, 2, 2),
    yu   = run.displayYukawaElement(YU, 1, 1), 
    yd   = run.displayYukawaElement(YD, 1, 1), 
    ye   = run.displayYukawaElement(YE, 1, 1),
    g1   = run.displayGaugeCoupling(1), 
    g2   = run.displayGaugeCoupling(2), 
    g3   = run.displayGaugeCoupling(3); 
  
  // gauge parts of beta functions
  double Xu = -13.0 / 15.0 * sqr(g1) - 3.0 * sqr(g2) - 16 * sqr(g3) / 3.0;
  double Xd =  -7.0 / 15.0 * sqr(g1) - 3.0 * sqr(g2) - 16 * sqr(g3) / 3.0;
  double Xe =  -9.0 / 5.0 * sqr(g1)  - 3.0 * sqr(g2);
  
  // Yukawa parts
  double betayt   = yt *   (Xu + 6.0 * sqr(yt) + sqr(yb));
  double betayb   = yb *   (Xd + 6.0 * sqr(yb) + sqr(yt) + sqr(ytau));
  double betaytau = ytau * (Xe + 3.0 * sqr(yb) + 4 * sqr(ytau));

  double betayu   = yu *   (Xu + 3.0 * sqr(yt));
  double betayd   = yd *   (Xd + 3.0 * sqr(yb) + sqr(ytau));
  double betaye   = ye *   (Xe + 3.0 * sqr(yb) + sqr(ytau));
  double betayc   = yc *   (Xu + 3.0 * sqr(yt));
  double betays   = ys *   (Xd + 3.0 * sqr(yb) + sqr(ytau));
  double betaymu  = ymu *  (Xe + 3.0 * sqr(yb) + sqr(ytau));
  ...
  double mursq = ONEO16pif * sqr(m32) * 
    (-88. / 25. * sqr(sqr(g1)) + 8 * sqr(sqr(g3)) + 2 * yu * betayu);
  double mdrsq = ONEO16pif * sqr(m32) * 
    (-22. / 25. * sqr(sqr(g1)) + 8 * sqr(sqr(g3)) + 2 * yd * betayd);
  double mQl1sq = ONEO16pif * sqr(m32) * 
    (-11. / 50. * sqr(sqr(g1)) - 1.5 * sqr(sqr(g2)) + 8 * sqr(sqr(g3)) 
     + yd * betayd + yu * betayu);
  double mL1sq = ONEO16pif * sqr(m32) * 
    (-99. / 50. * sqr(sqr(g1)) - 1.5 * sqr(sqr(g2)) + ye * betaye);
  double mersq = ONEO16pif * sqr(m32) * 
    (-198. / 25. * sqr(sqr(g1)) + 2.0 * ye * betaye);

  double mcrsq = ONEO16pif * sqr(m32) * 
    (-88. / 25. * sqr(sqr(g1)) + 8 * sqr(sqr(g3)) + 2 * yc * betayc);
  double msrsq = ONEO16pif * sqr(m32) * 
    (-22. / 25. * sqr(sqr(g1)) + 8 * sqr(sqr(g3)) + 2 * ys * betays);
  double mQl2sq = ONEO16pif * sqr(m32) * 
    (-11. / 50. * sqr(sqr(g1)) - 1.5 * sqr(sqr(g2)) + 8 * sqr(sqr(g3)) 
     + ys * betays + yc * betayc);
  double mL2sq = ONEO16pif * sqr(m32) * 
    (-99. / 50. * sqr(sqr(g1)) - 1.5 * sqr(sqr(g2)) + ymu * betaymu);
  double mmursq = ONEO16pif * sqr(m32) * 
    (-198. / 25. * sqr(sqr(g1)) + 2.0 * ymu * betaymu);
  
  mQLsq(1, 1) = mQLsq(1, 1) + mQl1sq;
  mQLsq(2, 2) = mQLsq(2, 2) + mQl2sq;
  mQLsq(3, 3) = mQLsq(3, 3) + mQl3sq;
  mURsq(1, 1) = mURsq(1, 1) + mursq;
  mURsq(2, 2) = mURsq(2, 2) + mcrsq;
  mURsq(3, 3) = mURsq(3, 3) + mstoprsq;
  mDRsq(1, 1) = mDRsq(1, 1) + mdrsq;
  mDRsq(2, 2) = mDRsq(2, 2) + msrsq;
  mDRsq(3, 3) = mDRsq(3, 3) + msbottomrsq;
  mLLsq(1, 1) = mLLsq(1, 1) + mL1sq;
  mLLsq(2, 2) = mLLsq(2, 2) + mL2sq;
  mLLsq(3, 3) = mLLsq(3, 3) + mL3sq;
  mSEsq(1, 1) = mSEsq(1, 1) + mersq;
  mSEsq(2, 2) = mSEsq(2, 2) + mmursq;
