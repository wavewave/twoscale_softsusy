...
double DoubleVector::product() const {
  double f = 1.0;
  int i; for (i=start; i<=end; i++) f *= x[i];
  return f;
}
...
double DoubleVector::max() const {
  ...
  int i; 
  for (i=displayStart(); i<=displayEnd();  i++)
    if (display(i) > m) m = display(i);
  return m;
}

double DoubleVector::max(int & p) const {
  double m = double(0);
  int i; 
  for (i=displayStart(); i<=displayEnd();  i++)
    if (display(i) > m) {
...
DoubleVector DoubleVector::sort() const {
...
DoubleVector DoubleMatrix::displayRow(int i) const {
  DoubleVector temp(displayCols());
  int j; for (j=1; j<=displayCols(); j++) temp(j) = display(i, j);
  return temp;
}

DoubleVector DoubleMatrix::displayCol(int i) const {
...
void DoubleMatrix::resize(int r, int c) {
#ifdef ARRAY_BOUNDS_CHECKING
  if (r < 1 || c < 1) {
  ostringstream ii;
  ii << "DoubleMatrix.resize called with incompatible length " << r << "," 
     << c << *this;
  throw(ii.str());
  }
#endif
  double **t = myDoubleMatrix(1, r, 1, c);
  int i, j; for (i=1; i<=r; i++) 
    for (j=1; j<=r; j++) {
      if (i <= rows && j<= cols) t[i][j] = x[i][j];
      else t[i][j] = double(0);
    }
  freeMyDoubleMatrix(x, 1, rows, 1, cols);
  setMat(t, r, c);
...
DoubleVector DoubleMatrix::sym2by2(double & theta) const  {
  ...
  if (fabs(mm(2, 1)) > maxtol || fabs(mm(1, 2)) > maxtol) {
    ...
    ii << "Maximum tolerance=" << maxtol;
    ii << "=" << maximum(fabs(temp(1)), fabs(temp(2))) << "*" << TOLERANCE 
       << "*" << 1.0e3 << endl;
    throw ii.str();
  }
...
ComplexMatrix::ComplexMatrix(const DoubleMatrix & m)
  ...
  Complex ** t = myComplexMatrix(1, rows, 1, cols);
  int i, j; 
  for(i=1; i<=rows;i++)
    for(j=1; j<=cols; j++)
      t[i][j] = Complex(m.display(i, j), 0.);
  setMat(t, rows, cols);
  freeMyComplexMatrix(t, 1, rows, 1, cols);
}
...
DoubleMatrix ComplexMatrix::realify() const {
  DoubleMatrix temp(rows, cols);
...
bool ComplexMatrix::isThereImag() const {
  DoubleMatrix temp(rows, cols);
  int i, j; 
  for (i=1; i<=rows; i++)
    for (j = 1; j<= cols; j++)
      if (x[i][j].real() > EPSTOL) {
	if (x[i][j].imag() > x[i][j].real() * EPSTOL) return true;
      }
      else if (x[i][j].imag() > EPSTOL) return true;
