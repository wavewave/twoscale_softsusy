...
void MssmSoftsusy::printLong() {
  ...
  cout <<" " << mu << " " 
    ...
    calcRunningMt() << " " <<
    displayPhys().mhiggs.display(1) << " " <<
    displayPhys().mhiggs.display(2) << " " <<
    displayPhys().mhiggs.display(3) << " " <<
    displayPhys().mhiggs.display(4) << " " <<
    displayPhys().thetaH << " " <<
...
void MssmSoftsusy::addCharginoLoop(double p, DoubleMatrix & mass) {
  ...
  for (i=1; i<=2; i++) 
	...
	if (k <= 2) {
	  ...
	  // H
	  b1p = b1(p, mch(k), forLoops.mhiggs(3), q);
	  b0p = b0(p, mch(k), forLoops.mhiggs(3), q);
	  sigmaL(i, j) = sigmaL(i, j) + 0.5 *
	  ...
	  // h
	  b1p = b1(p, mch(k), forLoops.mhiggs(1), q);
	  b0p = b0(p, mch(k), forLoops.mhiggs(1), q);
	  sigmaL(i, j) = sigmaL(i, j) + 0.5 *
	    (aPsiChih(i, k).conj() * aPsiChih(j, k) * b1p).real();
	  sigmaR(i, j) = sigmaR(i, j) + 0.5 *
	    (bPsiChih(i, k).conj() * bPsiChih(j, k) * b1p).real();
	  ...
	  // A0
	  b1p = b1(p, mch(k), forLoops.mhiggs(2), q);
	  b0p = b0(p, mch(k), forLoops.mhiggs(2), q);
	  sigmaL(i, j) = sigmaL(i, j) + 0.5 *
...
void MssmSoftsusy::charginos(int accuracy, double piwwtMS) {
  ...
  if (accuracy == 0) {
    mCh.asy2by2(physpars.thetaL, physpars.thetaR);
    return;
  }
...
void MssmSoftsusy::addNeutralinoLoop(double p, DoubleMatrix & mass) {
  ...
  bPsiChiZ = bPsiChiZ * n.hermitianConjugate();
...
void MssmSoftsusy::neutralinos(int accuracy, double piwwtMS, double pizztMS) {
  ...
  physpars.mneut(1) = mneut(1); physpars.mneut(2) = mneut2(2);
  physpars.mneut(3) = mneut3(3); physpars.mneut(4) = mneut4(4);
}
...
void MssmSoftsusy::calcDrBarPars() {
  ...
  int pos;
  eg.mhiggs(1) = temp.min(pos); eg.mhiggs(3) = temp.max(); 
  eg.mhiggs(2) = sqrt(mAsq); eg.mhiggs(4) = sqrt(mAsq + mw2);
...
double MssmSoftsusy::piZZT(double p, double q, bool usePoleMt) const {
  ...
  susyHiggs = susyHiggs - 
    sqr(cos(2.0 * thetaWDRbar)) * b22bar(p, tree.mhiggs(4), tree.mhiggs(4), q);
...
double MssmSoftsusy::piAA(double p, double q) const {// checked 30.07.03
 ...
 if (tree.mu(1, 3) == 0.0 || tree.mu(2, 3) == 0.0) {
  ...
  double    ctau    = cos(thetatau);
  double    g       = displayGaugeCoupling(2);
  double    mh0     = maximum(tree.mhiggs(1), EPSTOL); ///< protects vs zeros
  double    mHc     = maximum(tree.mhiggs(4), EPSTOL);
  double    mH     = maximum(tree.mhiggs(3), EPSTOL);
  double    mA      = maximum(tree.mhiggs(2), EPSTOL);
  double    gp      = displayGaugeCoupling(1) * sqrt(0.6);
  double cosb = cos(beta), cosb2 = sqr(cosb), cos2b = cos(2.0 * beta),
    sinb = sin(beta), sinb2 = sqr(sinb), sin2b = sin(2.0 * beta);
...
(double & mtopPole, double & mGPole, double & smu, double & mA, double & tanb, 
 ...
 double & m2) 
  ...
  mA       = store.displayPhys().mhiggs.display(2);
...
void MssmSoftsusy::headerHiggsSLHA(ostream & out, const char model[], 
  ...
  out << "        25    "; printRow(out, s.mhiggs(1)); out << "   # h0\n";
  // out << "         32   "; printRow(out, displayMz()); out << "   # MZ\n";
  out << "        35    "; printRow(out, s.mhiggs(3)); out << "   # H0\n";
  out << "        36    "; printRow(out, s.mhiggs(2)); out << "   # A0\n";
  out << "        37    "; printRow(out, s.mhiggs(4)); out << "   # H+\n";
...
void MssmSoftsusy::mixingSLHA(ostream & out, const char model[], 
					  const DoubleVector & pars, 
					  int sgnMu, double tanb, 
					  double qMax, 
					  int numPoints, double mgut, 
...
void MssmSoftsusy::flavourIndependentSLHA(ostream & out, const char model[], 
					  const DoubleVector & pars, 
					  int sgnMu, double tanb, 
					  double qMax, 
					  int numPoints, double mgut, 
...
void MssmSoftsusy::lesHouchesAccordOutput(ostream & out, char model[], 
					  const DoubleVector & pars, 
					  int sgnMu, double tanb, 
					  double qMax, 
					  int numPoints, double mgut, 
  ...
  out << "     5   "; printRow(out, twoset.displayMass(mBottom)); out << "   # mb(mb)\n";
  out << "     6   "; printRow(out, d.displayPoleMt()); out << "   # Mtop(pole)\n";
  out << "     7   "; printRow(out, twoset.displayMass(mTau)); out << "   # Mtau(pole)\n";
  ...
  flavourIndependentSLHA(out, model, pars, sgnMu, tanb, qMax, numPoints, 
			 mgut, altEwsb);
...
bool testLEPHiggs(const MssmSoftsusy & r, double error) {
  double Mh = r.displayPhys().mhiggs.display(1);
  Mh = Mh + error;
...
double MssmSoftsusy::smPredictionMW() const {
  double mh = displayPhys().mhiggs.display(1);
...
double MssmSoftsusy::twoLoopGm2(double amu1Loop) const {
  ...
  double tanb   = displayTanb();
  double mA0    = displayDrBarPars().mhiggs.display(2);
  double mh0    = displayDrBarPars().mhiggs.display(1);
  double mH0    = displayDrBarPars().mhiggs.display(3);
...
void MssmSoftsusyAltEwsb::rewsb(int sgnMu, double mt) {
    ...
    double mAsq =  sqr(displayDrBarPars().mhiggs.display(2));
