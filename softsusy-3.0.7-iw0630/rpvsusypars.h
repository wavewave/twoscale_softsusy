...
public:
  ...
  Tensor displayLambda(RpvCouplings) const;
  /// Returns all kappa values in a vector
  inline DoubleVector displayKappa() const { return kappa; }
  /// Conventional couplings: lambda_{ijk}
  ...
  inline RpvSusyPars displayRpvSusy() const;
...
inline RpvSusyPars RpvSusyPars::displayRpvSusy() const { return *this; }
...
public:
  ...
  RpvSoftPars displayRpvSoft() const { return *this; };

  /// Returns one of RPV \f$ h_{U,D,E} \f$ parameter tensors
  Tensor displayHr(RpvCouplings) const;
  /// Returns vector of \f$ m_{H_1 L_i}^2 \f$
  inline DoubleVector displayMh1lSquared() const { return mH1lsq; }
  /// Returns vector of $D$ parameters
  inline DoubleVector displayDr() const;
  /// Only outputs matrices with non-zero elements (ie with at least one
  /// element higher than tol)
  void checkOut(double tol) const;
...
inline DoubleVector RpvSoftPars::displayDr() const { return dr; }
