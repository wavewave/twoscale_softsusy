...
ostream & operator <<(ostream &left, const RpvSoftsusy & r) {
  ...
  left << HR;
  left << "neutrino masses:" << r.displayNeutrinoMasses();
  left << "sneutrino Vevs:" << r.displaySneutrinoVevs();
  left << "full 7 by 7 neutral fermion mixing" << r.displayNeutralMixing();
...
void RpvSoftsusy::rpvDisplay(DoubleVector & parameters) const {
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=j+1; k<=3; k++){
	parameters(pos) = displayLambda(LE)(i, j, k);
	pos++;
      }
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	parameters(pos) = displayLambda(LD)(i, j, k);
	pos++;
      }
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=j+1; k<=3; k++){
	parameters(pos) = displayLambda(LU)(i, j, k);
	pos++;
      }
  for (i=1; i<=3; i++){
...
void RpvSoftsusy::iterateChargedLeptons(double vev, DoubleMatrix & yeOld, 
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) {
      ...
      int x; for (x=1; x<=3; x++) 
	yeNew(i, j) = yeNew(i, j) + 
	  displayLam(x, i, j) * displaySneutrinoVevs()(x);
...
DoubleMatrix RpvSoftsusy::chargedLeptons(double vev) {
  ...
  DoubleMatrix m(5, 5);
  m(1, 1) = displayGaugino()(2);
  m(2, 1) = displayGaugeCoupling(2) * v1 / root2;
  m(1, 2) = displayGaugeCoupling(2) * v2 / root2;
  m(2, 2) = displaySusyMu();

  int i; for (i=1; i<=3; i++) {
    m(i+2, 2) = displayKappa()(i);
    m(i+2, 1) = displayGaugeCoupling(2) * displaySneutrinoVevs()(i) / root2;
  }

  int k; for (k=1; k<=3; k++) 
    for (i=1; i<=3; i++) 
      m(2, k+2) = m(2, k+2) + displayYukawaElement(YE, i, k) * 
	displaySneutrinoVevs()(i) / root2;
  ...
  int j; for (j=1; j<=3; j++) {
	...
	- displayYukawaElement(YE, j, k) * v1 / root2;
      for (i=1; i<=3; i++)
	m(j+2, k+2) = m(j+2, k+2) 
	  + displayLam(i, j, k) * displaySneutrinoVevs()(i) / root2;
    }
  }
...
void RpvSoftsusy::lesHouchesAccordOutput(ostream & out, char model[], 
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; printRow(out, displayLambda(LE)(k, i, j));
	out << "   # lambda_{" << i << j << k << "}\n";      
      }
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; printRow(out, displayLambda(LD)(k, i, j));
	out << "   # lambda'_{" << i << j << k << "}\n";      
      }
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; printRow(out, displayLambda(LU)(i, j, k));
	out << "   # lambda''_{" << i << j << k << "}\n";      
      }
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; printRow(out, displayHr(LE).display(k,i,j));
	out << "   # T_{" << i << j << k << "}\n";      
      }
  ...
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; printRow(out, displayHr(LD).display(k,i,j));
	out << "   # T'_{" << i << j << k << "}\n";      
      }
  ...
  for (i=1; i<=7; i++)
    for (j=1; j<=7; j++) {
      out << "  " << i << " " << j << "   "; printRow(out, displayNeutralMixing().display(i,j));
      out << "   # N_{" << i << j << "}\n";
    }
...
void RpvSoftsusy::doQuarkMixing(DoubleMatrix & mDon, 
				 DoubleMatrix & mUpq) {
  switch(MIXING) {
  case 1:       
    mUpq = ckm.transpose() * mUpq * ckm;
    break;
  case 2: 
    mDon = ckm * mDon * ckm.transpose(); 
    break;
  case 3:
    mUpq = ckm.transpose() * mUpq;
    break;
  case 4:
    mDon = ckm * mDon;
    break;
