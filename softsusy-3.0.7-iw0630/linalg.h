...
/// DoubleVector is of variable length, and contains double precision numbers. 
class DoubleVector {
private:
...
public:
  ...
  DoubleVector operator*(const DoubleVector &v);
  /// Acts on each element by element
  DoubleVector operator/(const DoubleVector &v);
  ...
  double product() const; ///< product of elements

  /// Obvious display functions
  inline double display(int i) const; ///< display one element
  ...
  double max(int & p) const; ///< maximum element in vector
  ...
  DoubleVector sort() const; ///< re-orders the vector so it's in increasing order
};
...
public:
  ...
  inline DoubleMatrix(const ComplexMatrix &m); ///< Constructor sets = m.real
  ...
  /// re-sizes the matrix, copies as many elements as possible and fills up
  /// any extras up with zeroes
...
public:
  ...
  /// Initialises with real parts
  ComplexMatrix(const DoubleMatrix &v);  
  ~ComplexMatrix();
  ...
  /// sees if there is any significant imaginary component in the matrix
  bool isThereImag() const;
  /// returns a real matrix
  DoubleMatrix realify() const;
};
...
inline double DoubleVector::display(int i) const {
#ifdef ARRAY_BOUNDS_CHECKING
  if (i < displayStart() || i > displayEnd()) { 
    ostringstream ii;
    ii << "Error: Requested display (" << i << 
	")th element of vector " << *this; 
    throw ii.str();
    }
#endif
  return x[i];
...
inline DoubleMatrix::DoubleMatrix(const ComplexMatrix &m) 
  : rows(m.displayRows()), cols(m.displayCols()) { 
  x = myDoubleMatrix(1, rows, 1, cols);
  int i, j; for (i=1; i<=rows; i++)
	      for (j = 1; j<= cols; j++)
		x[i][j] = m.display(i, j).real();
}
