...
double nextscan( double start, double end, double numscan, double i ) {
  double result ;
  result = start + (end-start)*( i / numscan );
  return result ; 
}

double nextlogscan( double start, double end, double numscan, double i ) {
  double result ;
  double logstart ; 
  double logend ;
  double logresult ; 
  ...
  logresult = logstart + (logend-logstart)*( i / numscan );
...
int main(int argc, char *argv[]) {
  ...
  if( argc == 3 ) { 
	...
	tanb     = randomgenerator( tanb_start,     tanb_end ) ; 
      ...
      if( inputmethod == "linear" ) {
	alpha    = nextscan( alpha_start,    alpha_end,    numscan, i ); 
	beta     = nextscan( beta_start,     beta_end,     numscan, i ); 
	M_moduli = nextscan( M_moduli_start, M_moduli_end, numscan, i ); 
	M_mess   = nextlogscan( M_mess_start,   M_mess_end,   numscan, i );
	tanb     = nextscan( tanb_start,     tanb_end,     numscan, i ); 
      }
	...
	cout << "test N11 = " <<p.mixNeut(1,1) << endl 
	     << "     N21 = " <<p.mixNeut(2,1) << endl
	     << "     N31 = " <<p.mixNeut(3,1) << endl
	     << "     N41 = " <<p.mixNeut(4,1) << endl;
