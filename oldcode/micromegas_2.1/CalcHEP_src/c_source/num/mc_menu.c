        ...
        case 5:
            ...
            && nPoints>=3 && nPoints<=150 && scale>0.5)
         {
           ...
           
           {
              ...
              strcpy(p_name,pinf_int(proces_1.nsub,mode,NULL,NULL));
              strcat(p_name,"(x)*x");
...
static int  in_setting(void)
{
  ...
  for(i=0;i<2;i++) 
  if(sf_num[i])mass[i]=sf_mass[i];else pinf_int(proces_1.nsub,i+1,mass+i,NULL);
...
/* **   menu loop */
  ...
  {    
    ...
    if(is_polarized(1,proces_1.nsub))
    ...
    if(is_polarized(2,proces_1.nsub))
    ...
    { case 0:
         ...
         else pinf_int(proces_1.nsub,i+1,mass+i,NULL);         
      ...
      case 4: correctDouble(50,12,"Enter new value ",&inP2,1); 
              returnCode=returnCode|1; break;
      case 5: if(is_polarized(1,proces_1.nsub))
              {  correctDouble(50,12,"Enter new value ",Helicity,1);
                 returnCode=returnCode|1;
              }    break;
      case 6: if(is_polarized(2,proces_1.nsub))
...
{  int i;
   ...
   {  
       pinf_int(proces_1.nsub,i,&m_,NULL);
       ms+=m_;
   }
 
   if(nin_int==1)
   { 
      pinf_int(proces_1.nsub,1,&m_,NULL);
      if(m_<=ms) return 1;
   } else 
   {                                                         
     double S=0,m1,m2;
     pinf_int(proces_1.nsub,1,&m1,NULL);
     pinf_int(proces_1.nsub,2,&m2,NULL); 
        ...
        case 11:
                ...
                {  void (*f10_tmp)(int);
                   ...
                   { double m1,m2,Pcm;
                     ...
                     if(sf_num[0]||sf_num[1]||invcut_1[0].key)messanykey(10,10,
