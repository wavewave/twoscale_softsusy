...
{ int fast=1;
  ...
  if(strcmp(mess,"~A")) printf(" ~A is not LSP\n"); 
...
printf("\n====== Calculation of widths and cross sections ====\n");  
  ...
  Helicity[0]=0.45;
  Helicity[1]=-0.45;
  printf("Process e1,E1->2*x at Pcm=%.3E GeV\n",Pcm);
  cc=newProcess("e1%,E1%->2*x","eE_2x");
  if(cc)
  { int ntot,l;
    char * name[4];
  ...
  printf("\n WIMP annihilation at rest\n");
 
  cc=newProcess("","omg__t_a__t_a");
