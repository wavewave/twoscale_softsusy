...
{  
   ...
   if(pInvolved[0].num==0) return 10;
   ...
   sprintf(command," cd %s/models; sed 1cAuxiliary\\ Model vars1.mdl  > vars2.mdl;"
   ...
   sprintf(command,"cd %s; rm -f tmp/safe; ./calchep -blind "
   "\"[{[[{{{MXXX\\091\\09\\09"

   "{sqrt3D\\091.732050807568877\\09\\09"
   
   "}}0\"",WORK);
   err=system( command);
   if(err) {free(command);return 3;}

   sprintf(command,"cd %s;  ./calchep -blind \"[[{[[{"
   ...
   sprintf(command,"cd %s; ./calchep -blind \"[[{[[[{",WORK);
   for(i=1;i<K;i++) 
   ...
   {  
      ...
      auxVert[1]="V5\\09\\092*MXXX/sqrt3D\\09G5*G(m3)\\09";
   }else 
   ...
   { 
      ...
      auxVert[0]=buff;
      auxVert[1]="V5\\09\\092*i*MXXX*Sqrt2\\09eps(m1,m2,(p1-p2),m3)\\09";
   }   
...
{ 
  ...
  return   parton_x(pdfQnum,sqrt(msq*msq-mne*mne))/D2*(D +sgn*2*p1_p2);
}
...
//if(ampl) printf("I=%d %E for %s %s -> %s %s\n",I,ampl, names[0],names[1],names[2],names[3]);      
      ...
      { if(aQnum<=3 ) 
        {  
         pA5[0]+=ampl*wV5P__[aQnum-1]/2;
         nA5[0]+=ampl*wV5N__[aQnum-1]/2; 
         pA5[1]+=sgn*ampl*wSM0P[aQnum-1]/2;
         nA5[1]+=sgn*ampl*wSM0N[aQnum-1]/2;                                     
        }
      }
      ...
      double * dNdE)
{
  ...
  const double step=1.E-6;                   /* 1KeV */
  const int NSTEP=200;

  int i;
  double MA,Mwimp,ffs,Rs,sum,css,csv00,csv01,csv11; 
  ...
  { double pA0[2],pA5[2],nA0[2],nA5[2];
    ...
    css*=SCcoeff/2;
    csv00*=4*M_PI*SCcoeff/(2*J+1)/2.;    
    csv11*=4*M_PI*SCcoeff/(2*J+1)/2.;
    csv01*=4*M_PI*SCcoeff/(2*J+1)/2.;
  }
/*printf("css=%E, csv00=%E, csv11=%E, csv01=%E\n",css,csv00,csv11,csv01);*/
  ...
  { double E=i*step;
    ...
    if(i==0 ||i==NSTEP-1) sum+=dNdE[i]/2; else sum+=dNdE[i];
  }
  return sum;
}

static double S00_0,S01_0,S11_0, RS_,NRS_;
static double S00_(double p)
{ double r;
  if(p<1.E-5) return S00_0;
  p_=p; R_=RS_; a_=0.52;
  r=simpson(FermiNDP,0., R_+10*0.5, 1.E-4)/p_/NRS_;
  return S00_0*r*r;
}

static double S01_(double p)
{ double r;
  if(p<1.E-5) return S01_0;
  p_=p; R_=RS_; a_=0.52;
  r=simpson(FermiNDP,0., R_+10*0.5, 1.E-4)/p_/NRS_;
  return S01_0*r*r;
}
...
double * dNdE)
{
  if(J)
  { double A3=pow(A,1./3.);
    RS_= 1.5*A3 - 3*(A3-5+ sqrt((A3-5)*(A3-5) +0.1) );
    a_=A_;
    ...
    R_=RS_; 
    NRS_=simpson(FermiND0,0., R_+10*0.5, 1.E-4);
  }  
  return nucleusRecoil(rho,vfv,A,Z,J,S00_,S01_,S11_,LF,dNdE);
}

/*====== Auxilarry service functions ======*/
int displayRecoilPlot(double * tab, char * text, int E1, int E2)
{ 
  FILE*f;
  int i;
  
  if(E1<0 || E2>199 || E1>=E2+1) return 1;
  f=fopen("_plot_","w");
  if(!f) return 2;
  ...
  for(i=E1;i<=E2;i++) fprintf(f,"%E\n",tab[i]);  
  fclose(f);
  system("trap \"rm -f _plot_\" 0 1 2 3 9; ../CalcHEP_src/bin/plot_view < _plot_ &");
  return 0;
}

double cutRecoilResult(double *tab, int E1, int E2)
{ int i;
  double sum;
  if(E2>199) E2=199;
  if(E1<0 || E1>=E2) return 0;
  for(i=E1,sum=0;i<=E2;i++) sum+=tab[i];
  sum-=(tab[E1]+tab[E2])/2;
  return sum;
}
...
{ double q_fm=q;
  ...
  return ((2*J+1)/8/M_PI)*(-0.214)*exp(-u)*(p01*q01*(1+be0)+p21*q21*(1+be2)
  -be02*(p01*q21+p21*q01));
} 
...
double static ss_exp_aux(double q, double A,double B)
{   double q_fm=q, y=0.25*pow(73,1./3.)*q_fm*q_fm;
    return A*exp(-B*y);
}    

/*================= Si29A ===================== */ 
double S00Si29A(double q)  {return ss_exp_aux(q, 0.00818,4.428);}   
double S11Si29A(double q)  {return ss_exp_aux(q,0.00818*1.06,6.264);}
double S01Si29A(double q)  {return ss_exp_aux(q,0.00818*(-2.06),5.413);}

/*================= Ge73A ===================== */
double S00Ge73A(double q){return ss_exp_aux(q,0.20313*1.102,7.468);}
double S11Ge73A(double q){return ss_exp_aux(q,0.20313,8.856);}
double S01Ge73A(double q){return ss_exp_aux(q,0.20313*(-2.099),8.191);} 
  
/*-------polinomial parametrization ----------------*/
 
static double SSxxYYzz(double q,double * data, int nData,int A,int expKey,double yN,double y_max)
{ int i;
  double q_fm=q, y=0.25*pow((double)A,1./3.)*q_fm*q_fm, y_i,res;
