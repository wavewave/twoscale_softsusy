...
*Local 
      ...
      COMPLEX*16 EGD(3,3), EUD(3,3)   ! Diagonal E_g and E_u
...
*      print*,'As(Mt^pole),As(M_sbottom) = ',asmt,as_msb
*YT(M_Stop)
...
*      print*,'YB(MT^pole),YT(MT^pole),YT(M_Stop) =',yb_mt,yt_mt,yt_mst
*EGD
      DO I=1,3
       DO J=1,3
...
*      print*,yu**2,yc**2,yt**2
      DO I=1,3
       DO J=1,3
        EUD(I,J)=DCMPLX(0.D0,0.D0)
       ENDDO
      ENDDO
      EUD(1,1)=1.D0/16.D0/PI**2*DCONJG(MU_H*AT_H)*YU**2
     .         *F_I(MU1**2,MQ1**2,CDABS(MU_H)**2)
      EUD(2,2)=1.D0/16.D0/PI**2*DCONJG(MU_H*AT_H)*YC**2
     .         *F_I(MU2**2,MQ2**2,CDABS(MU_H)**2)
      EUD(3,3)=1.D0/16.D0/PI**2*DCONJG(MU_H*AT_H)*YT**2
...
*
      EUD(1,1)=EUD(1,1)+FAC_2HDM*YU**2
      EUD(2,2)=EUD(2,2)+FAC_2HDM*YC**2
      EUD(3,3)=EUD(3,3)+FAC_2HDM*YT**2
*      print*,'EUD(1,1): ',eud(1,1)
*      print*,'EUD(2,2): ',eud(2,2)
*      print*,'EUD(3,3): ',eud(3,3)
*      print*,'EUD(3,3)/YT**2: ',eud(3,3)/yt**2
*-----------------------------------------------------------------------
*===>R and R^-1 
      DO I=1,3
       DO J=1,3
        RD(I,J)=DELTA(I,J)+TB_H*(EGD(I,J)+EUD(I,J))
*        RD(I,J)=DELTA(I,J)+TB_H*(EGD(I,J))
*        RD(I,J)=DELTA(I,J)+TB_H*(EUD(I,J))
       ENDDO
      ENDDO
*
...
*Checking R^-1*(EG+EU)=(1-R^-1)/tanb
      DO I=1,3
       DO J=1,3
        C33(I,J)=RDI(I,1)*(EGD(1,J)+EUD(1,J))
     .          +RDI(I,2)*(EGD(2,J)+EUD(2,J))
     .          +RDI(I,3)*(EGD(3,J)+EUD(3,J))
...
*      print*,'RBtau',RBTAUNU
*     .      ,M3_H/CDABS(M3_H),TB_MSUSY,DCONJG(GL_CH(3,1))/CKM(1,3)
      RAUX_H(134)=RBTAUNU
*-----------------------------------------------------------------------
*b -> s gamma
...
*      write(*,22) bdtt*1.d7,bdtt_sm*1.d7
      ...
      write(*,23) dmbd_susy
      write(*,24) dmbs_susy
      write(21, fmt='("Bsmumu",2x,1PE16.8)') bsmm
      write(21, fmt='("Bsg",2x,1PE16.8)') raux_h(135)*1.d-4
      write(21, fmt='("Bulnu",2x,1PE16.8)') rbtaunu
      write(21, fmt='("Bdll",2x,1PE16.8)') bdtt
      write(21, fmt='("ABsg",2x,1PE16.8)') raux_h(136)
      write(21, fmt='("deltMd",2x,1PE16.8)') dmbd_susy
      write(21, fmt='("deltMs",2x,1PE16.8)') dmbs_susy
...
*-----------------------------------------------------------------------
*Charged Higgs 
      AU_C=-DCONJG(GR_CH(2,3))/CKM(3,2)
      AD_C= DCONJG(GL_CH(3,3))/CKM(3,3)
*      print*,tb_h,au_c,ad_c
*For COMMON /C78_1_CH/ AU_CH,AD_CH
      AU_CH=AU_C
      AD_CH=AD_C
*      print*,'>>> MAIN',au_ch,ad_ch
*
      XTH=(MT_MW/MCH)**2
*      print*,mch,xth
      C7_0_MW_CH=CDABS(AU_C)**2/3.D0*B2SG_F7_1(XTH)
     .            -AD_C*DCONJG(AU_C)*B2SG_F7_2(XTH)
      C8_0_MW_CH=CDABS(AU_C)**2/3.D0*B2SG_F8_1(XTH)
     .            -AD_C*DCONJG(AU_C)*B2SG_F8_2(XTH)
...
*      print*,'C2_0 (mu_b) ',c2_0
*      print*,'C7_0 (mu_b) ',c7_0
*      print*,'C8_0 (mu_b) ',c8_0
*-----------------------------------------------------------------------
*C7_1 at RMU
*      print*,eta
...
************************************************************************
      ...
      COMMON /C78_1_CH/ AU_CH,AD_CH
*
      B2SG_G7_H=AD_CH*DCONJG(AU_CH)*4.D0/3.D0*X*(
...
*
      ...
      COMMON /C78_1_CH/ AU_CH,AD_CH
*
      B2SG_G8_H=AD_CH*DCONJG(AU_CH)*1.D0/3.D0*X*(
...
*
      ...
      COMMON /C78_1_CH/ AU_CH,AD_CH
*
      B2SG_D7_H=AD_CH*DCONJG(AU_CH)*2.D0/9.D0*X*(
...
*
      ...
      COMMON /C78_1_CH/ AU_CH,AD_CH
*
      B2SG_D8_H=AD_CH*DCONJG(AU_CH)*1.D0/3.D0*X*(
