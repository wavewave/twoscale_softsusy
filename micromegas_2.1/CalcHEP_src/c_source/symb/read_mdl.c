...
static int  rd_3(char* s, double *p )
{
   int i;
...
/*   if(strchr(s,'.')) return 0;   */
   ...
   if (strcmp(s,strongconst) == 0 || strcmp(s,"i")==0  ) return 0; 
            
   for(i=1;i<=nmodelvar;i++)
   { 
      if (strcmp(modelvars[i].varname,s) == 0) 
      {  
         if(modelvars[i].func) { *p=1; return -1;}
         else  {*p=modelvars[i].varvalue; return 1;}
      }
   }
   return 0;
}
...
static int  findvar(char* txt,double* num)
{
   int  i;
...
{  char      *ss,*endstr;
   ...
   {  ss=ln->line;
	 ...
	 prtclbase1=prtclbase-1;
      ...
      { 
         errcode=findvar(imassname,NULL);
...
/*  goto_xy(1,20); print("%d           ",nLine); */
  ...
  { int err;
    err=findvar (vardef->vars[n].name ,NULL );
    if (err)
