...
/* *    MASS INTEGRATION */
	...
	decay_0(nvin[i], amass[i][0], amass[i][1], &fct0);
	...
	*tfact = *tfact * fct0 / fct_1__;
    ...
    if(!finite(*tfact)){ *tfact=0; fprintf(stderr,"mkmom: infinite factor\n");}
    return 0;
}    
...
int imkmom(double P1, double P2)
{
    ...
    for (i=0; i <  nin_int + nout_int; i++) pinf_int(proces_1.nsub,i+1,pm+i,NULL);
