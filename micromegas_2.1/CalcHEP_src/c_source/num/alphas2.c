...
/* 
    double * alfPrt=&alphaMZ;

   for(i=1,i<=nvar_int;i++) if(strcmp("alfSMZ",varName_int[i])==0) 
   {  alfMZPrt=va_int+i; break;}
*/
   initStrFun(0);
L10:{  char strmen[]="\030"
...
static int qq1(char *s,double *v)
{
  ...
  if(isdigit(*s)) { sscanf(s,"%lf",v); return 1;}
  if(checkPhysVal(s,&key,plist)) *v=calcPhysVal(key,plist);
  else 
  { int i;
    for(i=1;i<=nvar_int+nfunc_int;i++)
    if(strcmp(varName_int[i],s)==0) *v=va_int[i];
  }
  return 1;
}
