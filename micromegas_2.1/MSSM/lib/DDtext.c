...
int MSSMDDtest(int loop, double*pA0,double*pA5,double*nA0,double*nA5)
{
  ...
  char mess[10];
  double  ALPE,SW,CW,MW,MZ,E,G,mne,beta,sb,cb;
  int i,II,IQ,i1,i2;
  double Ampl0,Ampl2;
  double MN=0.939;
...
/*========  Quark,SQUARK masses and mixing  ======*/
  for(IQ=1;IQ<=6;IQ++)
  { mq[IQ]=findParamW(MqNames[IQ-1]);
    mqSM[IQ]=findParamW(MqSMnm[IQ-1]);
    
    msq[0][IQ]=findParamW(MS1mass[IQ-1]);
    msq[1][IQ]=findParamW(MS2mass[IQ-1]);
    for(i1=0;i1<2;i1++) for(i2=0;i2<2;i2++)
    {
       sprintf(buffName,"%s%d%d", ZqNames[IQ-1],i1+1,i2+1);
       MI[i1][i2][IQ]=findParamW(buffName);
    }
    Aq[IQ]=findValW(AqNames[IQ-1]);
  }

  for(i=1;i<=4;i++){sprintf(buffName,"Zn1%d",i); NL[i]=findParamW(buffName);}
...
//****** Heavy squarks in case of loops    
  ...
  {  double f,g,bd,b1d,bs,b1s,b2s; 
     if(QCDcorrections ) qcdNLO=1+(11/4.-16./9.)*alphaQCD(mqSM[IQ])/M_PI;
     else qcdNLO=1;
...
//****** stop twist2 term 
     if(IQ==6)
     { double Ampl2;
       Ampl2=alphaQCD(mqSM[IQ])*(b2s+mne*(b1s+b1d)/2)*parton_x(21,mqSM[IQ]);     
...
//printf("IQ=%d, gp=%E gn=%E\n",IQ, g*parton_x(IQ,sqrt(D)),  g*parton_x(IQn,sqrt(D)));
    *pA0-=1.5*g*mne*MN*parton_x(IQ, sqrt(SQ(msq[II][IQ])-SQ(mne)));
    *nA0-=1.5*g*mne*MN*parton_x(IQn,sqrt(SQ(msq[II][IQ])-SQ(mne)));            
  }     
#endif
//******  higgs-quark-anitiquark
  for(IQ=1;IQ<=6;IQ++)
  { double fh,fH;
    if(QCDcorrections && IQ>3) qcdNLO=1+(11/4.-16./9.)*alphaQCD(mqSM[IQ])/M_PI;
     else qcdNLO=1;
