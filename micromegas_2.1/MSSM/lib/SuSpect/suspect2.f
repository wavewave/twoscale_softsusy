...
c  VERSION 2.34  
c  Last modifications : May 13, 2005
...
c --create Block MODSEL --
       imod(1) = 1
       imod(2) = ichoice(1)
       if(imod(2).le.2) modselval = 'general MSSM'
       if(imod(2).eq.10) modselval = 'mSUGRA'
       if(imod(2).eq.11) modselval = 'GMSB'
       if(imod(2).eq.12) modselval = 'AMSB'
