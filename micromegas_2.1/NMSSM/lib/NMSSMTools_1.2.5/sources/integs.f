...
*   IFAIL=4 max(|dy(i)/dx|*(1+x)/1+|y(i)|)) > 1/eps^2
	...
	DO NSTP=1,MAXSTP
	 ...
	 DO I=1,NVAR
	  IF((1.D0+DABS(Y(I)))/(1.D0+X)/DABS(DYDX(I)).LT.EPS**2)IFAIL=4
	 ENDDO
	 ...
	 CALL RKQSS(Y,DYDX,NVAR,X,H,EPS,YSCAL,HDID,HNEXT,DERIVSS,IFAIL)
...
*   to be integrated by ODEINTS
	...
	F(8)= HTAU2*(-3.D0*g1 - 3.D0*g2
     ...
     .	    + HB2*(16*G3- 2.D0/3.D0*G1)+75.D0/2.D0*G1**2
	...
	SUBROUTINE RKQSS(Y,DYDX,N,X,HTRY,EPS,YSCAL,HDID,HNEXT,DERIVSS,
     .			IFAIL)
